...
prob1a.fun(5,4)
e <- prob1a.fun(5, 4)
ifelse(e==20, "Correct Answer Obtained", "Incorrect Answer Obtained")
```
...
duniform <- function(x, a, b){
  ...
  for(i in 1:length(x)){
  if(x[i]<a){output[i]<-0
  }else if(x[i]<b){output[i]<-1/(b-a)
      }else if(b<x[i]){output[i]<-0}
  }
  return(output)}
```
...
duniform2 <- function(x, a, b){
  ifelse(a<x & x<b, output<-1/(b-a), output<-0)
}
...
FizzBuzz <- function(x){
  ...
  }else  if(x[i]%%3==0){output[i] <- 'Fizz'
  }else if(x[i]%%5==0){output[i]<- 'Buzz'
  }else {output[i] <- i}
  }
    return(output)
  }
