...
```{r}
starwars <- read_excel("Z:/STAT445/starwars.xlsx", sheet=2, range='A3:K90')
head(starwars)
```
...
```{r}
starwars1 <- drop_na(starwars, mass)
starwars2 <- drop_na(starwars1, height)
ggplot(starwars2, aes(x=height, y=mass))+
  geom_point()+
  labs(title='StarWars Character Mass by Height')+
...
```{r}
starwars3 <- starwars2 %>% filter(mass<500)
ggplot(starwars3, aes(x=height, y=mass))+
  geom_point()+
  labs(title='StarWars Character Mass by Height')+
  labs(x="Height(cm)", y="Mass(kg)")
```
...
```{r}
HtWtModel <- lm( mass ~ height, data=starwars3)
...
```{r}
starwars4 <- starwars3 %>% drop_na(gender)
HtWtModel2 <- lm( mass ~ height, data=starwars4)

ggplot(HtWtModel, aes(x=height, y=mass))+
  geom_point()+
  labs(title='StarWars Character Mass by Height')+
  labs(x="Height(cm)", y="Mass(kg)")

starwars4 <- starwars4 %>% mutate(
  Ht_Inches = height*0.393701
)
```
...
starwars_Summ <- starwars4 %>% group_by(gender) %>% 
  summarise(
  AvgHt = mean(height)
)
starwars_Summ
...
```{r}
starwars_hair <- starwars %>% drop_na(hair_color) %>%
  mutate(
  result = str_replace(hair_color, '$, grey', '')
)
starwars_hair$hair_color
```
...
```{r}
Yavin <- ymd('1999-December-7')
starwars_years <- starwars4 %>% drop_na(birth_year) %>% mutate(
  DateOfBirth = Yavin-birth_year
)
starwars_years
